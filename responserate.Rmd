...
kable(s[, length(unique(cid)),by=responder])
```
...
```{r fig.caption="Response Rate by Day"}
...
## dotplot( branch ~ beta | parm, function(x,y,subscripts,...){
##     panel.dotplot(x,y,...)
##     d <- j[subscripts,]
##     for(i in 1:nrow(d)){
##         f <- d[i,]
##         panel.lines( c(f$"p2.5",f$"p97.5"),rep(y[i],2),lwd=2)
##     }
## },data=j, xlim=list(c(-.02,.02),c(0.05,0.6),c(0,0.002),c(0,0.03)),
## xlab='Beta', ylab='branch',scale=list(x=list(relation='free')),cex=2)
...
kable(apply(s2[!is.na(newprofile), parms2, with=FALSE],2,mean,na.rm=TRUE))
```
...
```{r conc1,echo=FALSE,fig.cap='Tables of Responsders, Hours(28 days) and distributions'}
invisible({
       s3 <- s2[!is.na(newprofile),];s3$id <- 1
       s3[, thCut:=cut(th, quantile(th, 0:5/5), include.lowest=TRUE)]
   })
s3[, prop.table(xtabs(id ~ thCut+responder),2)*100]
kable((s3[, list(responseRate = mean(responder=='r'))
...
```{r conc2,echo=FALSE,fig.cap='Tables of Responders and OS and distributions'}
s3[, prop.table(xtabs(id ~ os+responder),2)*100]
kable((s3[, list(responseRate = mean(responder=='r')), by=os]))
```
...
```{r conc3,echo=FALSE,fig.cap='Table of Responders and Countries and distributions'}
kable(s3[, prop.table(xtabs(id ~ country2+responder),2)*100])
kable(s3[, list(responseRate = mean(responder=='r')), by=country2])
#           ,options = list(dom = 't',scrollX = TRUE))
```
...
```{r conc4,echo=FALSE, fig.cap='Table of New Profiles and Responder and distributions'}
kable(s3[, prop.table(xtabs(id ~ newprofile+responder),2)*100])
kable(s3[, list(responseRate = mean(responder=='r')), by=newprofile])
#           ,options = list(dom = 't',scrollX = TRUE))
```
...
```{r conc5,echo=FALSE,dependson='responseModel', fig.cap='Model Quality (poor)'}
library(knitr)
